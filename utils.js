...
export function drawVector2d_origin({
  ctx,
  canvas,
  squareSize,
  vec = { x: 0, y: 0 },
  ...
  ctx.lineTo(
    offset.x + unit.x * scale + norm.x * arrowWidth,
    offset.y - unit.y * scale - norm.y * arrowWidth
  );
...
export function drawVector2d_from({
  ctx,
  canvas,
  squareSize,
  vec = { x: 0, y: 0 },
  ...
  const offset = {
    x: canvas.width / 2 + squareSize * from.x,
    y: canvas.height / 2 - squareSize * from.y,
  };
  ctx.strokeStyle = vecColor;
  ctx.fillStyle = vecColor;
  ctx.lineWidth = lineWidth;
...
export function matMult(A, B) {
  if (A[0].length !== B.length)
    throw new Error(
      "Dimensions of the matrix do not match. Number of Cols of A (A[0].length) is not equal to number of Rows in B (B.length). Matrix should be in form A[row][col]."
    );
...
export function rref(A) {
  ...
  for (
    let i = 0;
    i < (cA.length > cA[0].length ? cA[0].length : cA.length);
    i++
  ) {
...
export function extend(A, B) {
  ...
  if (cA.length !== B.length)
    throw new Error(
      `Number of rows in B must match number of rows in A.\n- A: ${cA}\n- B: ${B}`
    );
  for (let i = 0; i < cA.length; i++) {
    for (let k = 0; k < B[0].length; k++) cA[i].push(B[i][k]);
  }
  return cA;
}
...
export function inv(A) {
  ...
  for (let i = 0; i < A.length; i++) {
    if (cA[i][i] === 0) throw new Error(`Matrix is singular.`);
  }
...
export function matSlice(
  ...
  if (
    ...
    start[0] > A.length - 1 ||
    start[1] < 0 ||
    start[1] > A[0].length - 1 ||
    end[0] > A.length - 1 ||
    end[1] > A[0].length - 1
  )
    throw new Error(
      `Invalid slice. Slices must have two components [i, y], be non-negative, within the range of indices of A, and start >= end.`
    );
